...
namespace Microsoft.AspNetCore.Components.Testing
{
    ...
    internal class Htmlizer
    {
        ...
        {
            ArrayRange<RenderTreeFrame> frames = renderer.GetCurrentRenderTreeFrames(componentId);
            HtmlRenderingContext context = new HtmlRenderingContext(renderer);
            int newPosition = RenderFrames(context, frames, 0, frames.Count);
        ...
        {
            ref RenderTreeFrame frame = ref frames.Array[position];
            List<string> result = context.Result;
            result.Add("<");
            result.Add(frame.ElementName);
            int afterAttributes = RenderAttributes(context, frames, position + 1, frame.ElementSubtreeLength - 1, out string capturedValueAttribute);
            ...
            if (remainingElements > 0)
            {
                ...
                bool isSelect = string.Equals(frame.ElementName, "select", StringComparison.OrdinalIgnoreCase);
                if (isSelect)
                {
                    context.ClosestSelectValueAsString = capturedValueAttribute;
                }

                int afterElement = RenderChildren(context, frames, afterAttributes, remainingElements);
